...
export async function writeToCloudflareKV(zipIndex, allJobs) {
  ...
  for (const batch of batches) {
    const kvPairs = batch.map(zip => ({
      ...
      expiration_ttl: 90000 // ~25 hours - expires before next run
    }));
  ...
  console.log(`    KV write complete: ${written} zip entries + metadata`);
  return { written };
}
